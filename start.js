...
const BOT_ENTRY = fileURLToPath(new URL("./index.js", import.meta.url));
...
async function stopBot(child = bot) {
...
async function restartBot() {
  if (DASHBOARD_ONLY || restarting) return false;
  restarting = true;
...
<body>
<div class="wrap">
  <div class="top"><div class="title"><h1>Arnel Control Center</h1><p>Obrolan, memory, dan trainer Arnel · <span id="styleCount">memuat contoh gaya</span></p></div><div class="topright"><button id="restart" class="btn">Restart Arnel</button><div id="status" class="pill starting">starting</div></div></div>
...
<script>
const $=id=>document.getElementById(id);let statusData=null;let settingsDirty=false;let brainData={memories:[],training:[],exchanges:[]};let historyStack=[null];let historyIndex=0;let historyPage={rows:[],hasMore:false,nextBeforeId:null};
...
function memories(rows){if(!rows.length)return '<div class="empty">belum ada memory</div>';return rows.map((x,i)=>'<div class="item"><div class="meta">uses '+x.uses+' · '+esc(time(x.updatedAt))+'</div><div class="text">'+esc(x.content)+'</div><div class="actions"><button class="btn danger small" onclick="delMemory('+i+')">Delete</button></div></div>').join('')}
...
['proEnabled','proTimes','proMax'].forEach(id=>$(id).addEventListener('input',()=>{settingsDirty=true}));
$('savePro').onclick=async()=>{let times=$('proTimes').value.trim();let max=Number($('proMax').value);if(!/^\d{2}:\d{2}(,\d{2}:\d{2})*$/.test(times)){alert('Format jadwal contoh: 08:00,12:30,19:30');return}await post('/api/proactive',{proactiveEnabled:$('proEnabled').checked,proactiveTimes:times,proactiveDailyMax:max});settingsDirty=false;setTimeout(refreshStatus,900)};
...
async function delMemory(i){const {chatId,id}=brainData.memories[i];if(!confirm('Hapus memory ini?'))return;await post('/api/memory',{action:'delete',chatId,id});await refreshBrain()}window.delMemory=delMemory;
...
async function teach(i){let x=brainData.exchanges[i],out=prompt('Jawaban Arnel yang seharusnya:',x.output);if(!out||!out.trim())return;await post('/api/train',{chatId:x.chatId,exchangeId:x.id,input:x.input,output:out.trim(),source:'teach'});await Promise.all([refreshBrain(),refreshStatus()])}window.teach=teach;
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && req.headers.origin && req.headers.origin !== url.origin) {
      return sendJson(res, 403, { ok: false, error: "Origin dashboard tidak cocok" });
    }
    ...
    if (url.pathname === "/api/train" && req.method === "POST") {
      ...
      if (source === "teach" && body.exchangeId) {
        correctDashboardExchange(body.chatId, Number(body.exchangeId), String(body.output));
      } else {
    ...
    if (url.pathname === "/api/proactive" && req.method === "POST") {
      ...
      if (!/^(?:[01]\d|2[0-3]):[0-5]\d(?:,(?:[01]\d|2[0-3]):[0-5]\d)*$/.test(times)) throw new Error("format jadwal tidak valid");
...
server.on("error", (error) => {
  console.error(error.code === "EADDRINUSE"
    ? `Port ${PORT} sudah dipakai. Hentikan dashboard lama atau ubah DASHBOARD_PORT di .env.`
    : `Dashboard gagal dibuka: ${error.message}`);
  process.exitCode = 1;
});
server.listen(PORT, HOST, () => {
  console.log(`Dashboard aktif di http://localhost:${server.address().port}`);
  if (DASHBOARD_ONLY) {
    state.botProcess = "stopped";
    state.whatsapp = "offline";
    console.log("Mode dashboard-only: bot WhatsApp tidak dijalankan.");
  } else spawnBot();
});
...
async function shutdown(signal) {
  ...
  await Promise.all([stopBot(), new Promise((resolve) => server.close(resolve))]);
  process.exit(0);
}
